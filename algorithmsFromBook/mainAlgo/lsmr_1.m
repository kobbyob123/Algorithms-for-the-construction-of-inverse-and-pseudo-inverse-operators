function [x, istop, itn, normr, normAr, normA, condA, normx, error_history]...
   = lsmr_1(A, b, lambda, atol, btol, conlim, itnlim, localSize, show, x_groundtruth)
% LSMR_TRACKED   Modified LSMR to track error against ground truth.
%
%   See original lsmr.m for full documentation.
%   
%   New Input:
%   x_groundtruth : (Optional) The true solution vector. If provided,
%                   the function calculates norm(x - x_groundtruth)
%                   at every iteration.
%
%   New Output:
%   error_history : Vector containing the error norm at each iteration.
  ...
  % Check if ground truth is provided for error tracking
  if nargin < 10 || isempty(x_groundtruth)
      x_groundtruth = [];
      track_error = false;
  else
      track_error = true;
  ...
  while itn < itnlim
    ...
    % --- INSERTED ERROR TRACKING ---
    if track_error
        error_history(itn) = norm(x - x_groundtruth);
    end
    % -------------------------------
  ...
  % Trim error history if we finished early
  if track_error
      error_history = error_history(1:itn);
  end
