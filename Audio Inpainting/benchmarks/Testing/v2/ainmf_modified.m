...
% AINMF_MODIFIED — ainmf.m with pluggable solvers for thesis benchmarking
%
...
for i = 1:maxit
    ...
    if strcmpi(method, 'am')
        ...
        % Diagnostic arrays — per-frame solver comparison data
        lsmr_istop_all = zeros(N, 1);
        lsmr_itn_all   = zeros(N, 1);
        % Per-frame error between backslash and lsmr_op solutions
        % Only populated when solver_ = 'compare' (Task 1 diagnostic mode)
        compare_err_z   = zeros(N, 1);   % ||z_lsmr - z_bsl|| / ||z_bsl||
        compare_err_s   = zeros(N, 1);   % ||s_lsmr - s_bsl|| / ||s_bsl||
        compare_itn     = zeros(N, 1);   % LSMR iterations used per frame
        compare_istop   = zeros(N, 1);   % LSMR stop code per frame
        ...
        parfor n = 1:N
            ...
            if sum(mmask(:, n)) == M
                ...
                %    Uses backslash result for the actual inpainting.
                %    This mode is diagnostic only — do not use for benchmarking.
                % ----------------------------------------------------------
                if strcmp(solver_, 'compare')
                    % Reference: backslash
                    z_bsl = MTDTM \ y_obs;

                    % Cholesky
                    R_c   = chol(real(MTDTM));
                    z_chol = R_c \ (R_c' \ y_obs);

                    % LSMR matrix
                    [z_lsm, ~, ~, itn_lsm] = mod_lsmr(MTDTM, y_obs, 0, ...
                        lsmr_atol_, lsmr_atol_, 1e8, lsmr_itnlim_, ...
                        0, false, []);
                    ...
                    A_op = @(v, mode) apply_A_op( ...
                        v, mmask(:,n), V(:,n), Top_, Uop_, M_);
                    [z_lop, ~, istop_n, itn_n] = mod_lsmr(A_op, y_obs, 0, ...
                        lsmr_atol_, lsmr_atol_, 1e8, lsmr_itnlim_, ...
                        0, false, []);

                    % Record errors relative to backslash reference
                    if norm(z_bsl) > 0
                        compare_err_z(n) = norm(z_lop - z_bsl) / norm(z_bsl);
                    end
                    s_bsl = DTM * z_bsl;
                    s_lop = DTM * z_lop;
                    if norm(s_bsl) > 0
                        compare_err_s(n) = norm(s_lop - s_bsl) / norm(s_bsl);
                    end
                    compare_itn(n)   = itn_n;
                    compare_istop(n) = istop_n;

                    % Use backslash result for inpainting
                    z = z_bsl;

                else
                    % Normal operation — single solver chosen by solver_ param
                    switch solver_
                        case 'backslash'
                            z = MTDTM \ y_obs;
                        ...
                        case 'lsmr_matrix'
                            [z, ~, istop_n, itn_n] = mod_lsmr(MTDTM, y_obs, 0, ...
                                lsmr_atol_, lsmr_atol_, 1e8, lsmr_itnlim_, ...
                                0, false, []);
                        ...
                        case 'lsmr_op'
                            A_op = @(v, mode) apply_A_op( ...
                                v, mmask(:,n), V(:,n), Top_, Uop_, M_);
                            [z, ~, istop_n, itn_n] = mod_lsmr(A_op, y_obs, 0, ...
                                lsmr_atol_, lsmr_atol_, 1e8, lsmr_itnlim_, ...
                                0, false, []);
            ...
            lsmr_istop_all(n) = istop_n;
            lsmr_itn_all(n)   = itn_n;
        end
        ...
        % ------------------------------------------------------------------
        if strcmp(solver_, 'compare')
            % Task 1: report per-frame lsmr_op vs backslash errors
            active = compare_err_z > 0;   % frames that went through the solver
            if any(active)
                fprintf('  AM iter %d | compare mode | frames=%d | ', i, sum(active));
                fprintf('err_z: mean=%.2e max=%.2e | ', ...
                    mean(compare_err_z(active)), max(compare_err_z(active)));
                fprintf('err_s: mean=%.2e max=%.2e | ', ...
                    mean(compare_err_s(active)), max(compare_err_s(active)));
                fprintf('itn: mean=%.1f max=%d | istop7=%d\n', ...
                    mean(compare_itn(active)), max(compare_itn(active)), ...
                    sum(compare_istop(active) == 7));
            end
        elseif strcmp(solver_, 'lsmr_matrix') || strcmp(solver_, 'lsmr_op')
            % Normal LSMR mode: report itnlim hits
            n_itnlim = sum(lsmr_istop_all == 7);
            n_lsmr   = sum(lsmr_istop_all > 0);
            if n_lsmr > 0
                fprintf('  AM iter %d: itnlim hit in %d/%d LSMR frames (avg itn=%.1f, max itn=%d)\n', ...
                    i, n_itnlim, n_lsmr, mean(lsmr_itn_all(lsmr_itn_all>0)), max(lsmr_itn_all));
            end
        end
    end
